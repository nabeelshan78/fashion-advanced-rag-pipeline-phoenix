...
# Initialize chatbot
chat_bot = ChatBot(generator_function=generate_params_dict)

def chatbot_interface(user_message, history):
    """
    Handles Gradio UI <-> ChatBot connection.
    - Sends user_message to ChatBot
    - Streams back assistant response
    - Updates Gradio Chatbot UI with chat_bot.conversation
    """
    if history is None:
        history = []
    ...
    # Convert ChatBot conversation into Gradio's [user, assistant] pairs
    conversation_pairs = []
    for i in range(len(chat_bot.conversation)):
        msg = chat_bot.conversation[i]
        if msg["role"] == "user":
            # Next assistant message (if exists) is reply
            if i+1 < len(chat_bot.conversation) and chat_bot.conversation[i+1]["role"] == "assistant":
                conversation_pairs.append([msg["content"], chat_bot.conversation[i+1]["content"]])

    # Stream bot reply character by character
    last_user, _ = conversation_pairs[-1]
    conversation_pairs[-1][1] = ""
    for char in bot_reply:
        conversation_pairs[-1][1] += char
        time.sleep(0.02)
        yield conversation_pairs

# --- Styling (light theme with accents) ---
custom_css = """
.gradio-container {
...
with gr.Blocks(theme=gr.themes.Base(), css=custom_css) as demo:
    with gr.Column(elem_id="header"):
        gr.Markdown("# 👗 Fashion Forward Hub\n### Your AI Shopping Assistant")

    chatbot = gr.Chatbot(
        value=[[chat_bot.initial_message["content"], None]],  # start with initial assistant msg
        elem_id="chatbot",
        label="Chat",
    ...
    with gr.Group(elem_id="input_group"):
        with gr.Row():
            msg = gr.Textbox(
                show_label=False,
                placeholder="💬 Ask me about styles, products, or trends...",
                autofocus=True,
                container=False
            )
            submit = gr.Button("➤", elem_id="submit_button", scale=0)

    gr.Examples(
        examples=[
            "Show me some popular summer dresses",
            "Do you have men’s running shoes under $100?",
            "What jackets are in style right now?",
            "Find me 3 formal shirts for office wear.",
            "Can you recommend something for winter travel?"
        ],
        inputs=msg
    )
    ...
    submit.click(chatbot_interface, [msg, chatbot], chatbot)
